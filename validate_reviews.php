	...
	function sbma_validate_reviews($form)
	{
		...
		$lms_course_name = urldecode(filter_input(INPUT_GET, 'lms_course_name', FILTER_SANITIZE_STRING));
		...
		foreach ($form['fields'] as &$field) {
			...
			if ($field->id == $FIELD_USER_ID && is_user_logged_in()) { // Field ID 8 = User ID
				$field_value = $user->ID;
			}
	
			// Validate and correct lms_course_id and lms_course_name
			if ($field->id == $FIELD_LMS_COURSE_NAME) { // Field ID 11 = Course Completed
			...
			if ($field->id == $FIELD_LMS_COURSE_ID) { // Field ID 7 = Course ID
				if (!empty($lms_course_name) && isset($course_mappings[$lms_course_name])) {
